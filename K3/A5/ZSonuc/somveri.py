...
for i in range(0,len(veri2)):#[tarih,%degisim,kapanıs,m5-k%,m10-k%,m15-k%,m20-k%]
    ...
    deger=veri2[i][6]-veri2[i][2]#m20-k
    yzd = (100.0*deger)/veri2[i][2]
    veri3[i][6]=yzd
...
for i in range(0,len(veri2)):
    ...
    deger=veri2[i][3]-veri2[i][6]#m5-m20
    yzd=(100.0*deger)/veri2[i][3]
    veri3[i].append(yzd)
    ...
    deger=veri2[i][4]-veri2[i][6]#m10-m20
    yzd=(100.0*deger)/veri2[i][4]
    veri3[i].append(yzd)
...
#o yüzden benzetimaralik == 5 olarak aliniyor 
veri4=[]


benzetimaralik=5 #bu kısmı her aralık için degistir
...
for i in range(0,len(veri3),benzetimaralik):
    for k in range(0,benzetimaralik):
        if (len(veri3)==i+k):#20 son aldığımız aralık olduğu için burda -20 demek lazım
            break
        else:
...
dosya=open("veriTest.txt","w")
st=""

for i in range(len(veri3)-benzetimaralik,len(veri3)):
    for k in range(0,len(veri3[i])):
        st=st+str(veri3[i][k])+","
st=st+str("Tarih")
st=st+"\n"
dosya.write(st)
